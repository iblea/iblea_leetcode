...
public:
    long long gridGame(vector<vector<int>>& grid) {
        ...
        vector<long long> top_sum(xlen + 1, 0);
        vector<long long> bottom_sum(xlen + 1, 0);
        ...
        // 첫 번째 로봇이 i 번째에서 밑으로 내려감.
        for (int i = 0; i < xlen; i++) {
            // top_sum[xlen] - top_sum[i+1] : 두번째 로봇이 얻을 수 있는 1번째 라인 점수
            // bottom_sum[i] : 두번째 로봇이 얻을 수 있는 2번째 라인 점수
            // 두번째 로봇이 얻을 수 있는 점수 중 최대값을 찾음.
            // 두 값 중 최대값이 두 번째 로봇이 얻을 수 있는 점수 중 최대값이 됨.
            // 이 값을 min 해서 첫 번째 로봇이 수집하고 난 뒤의 점수 중 최대값을 구하도록 함.
            result = min(result, max(top_sum[xlen] - top_sum[i+1], bottom_sum[i]));
        }
        return result;
    }
};
